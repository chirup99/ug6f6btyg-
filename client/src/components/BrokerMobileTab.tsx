  ...
  return (
    <div className={`fixed inset-0 z-40 flex flex-col pb-20 transition-colors duration-300 ${viewingSecondary ? "bg-violet-50 dark:bg-violet-950" : "bg-white dark:bg-gray-950"}`}>
      ...
      <div className="flex-1 flex flex-col min-h-0">
        {subTab === "orders" ? (
          <>
            {/* Toolbar: order count + Save to Journal — fixed above list */}
            {currentOrders.length > 0 && (
              <div className={`flex items-center justify-between px-4 pt-2.5 pb-2 flex-shrink-0 ${viewingSecondary ? "bg-violet-50 dark:bg-violet-950" : "bg-white dark:bg-gray-950"}`}>
                ...
                {!viewingSecondary && recordAllBrokerOrders && (
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 text-[11px] font-medium bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors active:scale-95"
            ...
            <div className="flex-1 overflow-y-auto custom-thin-scrollbar px-4 pb-3 space-y-1.5">
              {currentOrders.length === 0 ? (
