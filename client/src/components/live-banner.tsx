...
export function LiveBanner() {
  ...
  const [newsStockPrices, setNewsStockPrices] = useState<{
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setCurrentIndex((p) => (p + 1) % bannerCount),
      7000,
    );
  ...
  useEffect(() => {
    ...
    const t = setInterval(
      () => setActiveNewsIndex((p) => (p + 1) % newsItems.length),
      3500,
    );
    return () => clearInterval(t);
  }, [newsItems.length]);
  ...
  const fetchNifty50BannerNews = useCallback(async () => {
    ...
    try {
      ...
      const symbols = [
        ...new Set(top.map((i) => i.symbol).filter(Boolean)),
      ] as string[];
      if (symbols.length > 0) {
        fetch(
          `/api/news-stock-prices?symbols=${symbols.slice(0, 10).join(",")}`,
        )
          .then((r) => r.json())
          .then((priceData: any) => {
  ...
  const handleCnbcStream = () =>
    updateStreamUrl(
      "https://www.youtube.com/embed/P857H4ej-MQ?enablejsapi=1&modestbranding=1&controls=0&showinfo=0&rel=0",
    );
  ...
  const updateStreamUrl = (url: string) => {
    ...
    window.dispatchEvent(
      new CustomEvent("livestream-url-updated", { detail: { url: cleanUrl } }),
    );
  };
  ...
  useEffect(() => {
    ...
    if (saved)
      setYoutubeUrl(
        saved.includes("?")
          ? `${saved}&modestbranding=1&controls=0&showinfo=0&rel=0`
          : `${saved}?modestbranding=1&controls=0&showinfo=0&rel=0`,
      );
  }, []);
  ...
  useEffect(() => {
    const handler = (e: CustomEvent) => {
      ...
      if (url) {
        setYoutubeUrl(
          url.includes("?")
            ? `${url}&modestbranding=1&controls=0&showinfo=0&rel=0`
            : `${url}?modestbranding=1&controls=0&showinfo=0&rel=0`,
        );
      } else {
        setYoutubeUrl(
  ...
  const activeNewsItem =
    newsItems.length > 0 ? newsItems[activeNewsIndex % newsItems.length] : null;
  const visibleNews = activeNewsItem ? [activeNewsItem] : [];

  return (
    <Card className="w-full h-32 md:h-36 relative overflow-hidden bg-card border border-border">
            ...
            {!isVideoPlaying && (
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                <div className="absolute top-2 right-2 flex items-center gap-1 z-10">
                  <StreamMenu
                    onCnbc={handleCnbcStream}
                    onCustom={() => setIsUrlDialogOpen(true)}
                  />
                </div>
              </div>
            )}
        ...
        {currentIndex === 1 && (
          <div className="absolute inset-0 bg-gradient-to-r from-slate-900 via-violet-950/80 to-purple-950 flex items-stretch px-2 py-2 gap-2">
            ...
            <div className="hidden sm:flex flex-col w-[36%] shrink-0 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm px-2.5 py-1.5 relative overflow-hidden">
              ...
              <div
                className={`flex items-center gap-0.5 text-[10px] font-semibold relative z-10 ${isUp ? "text-emerald-400" : "text-red-400"}`}
              >
                {isUp ? (
              ...
              <div className="flex flex-col flex-1 min-h-0 overflow-hidden relative z-10 justify-center">
                {newsLoading || visibleNews.length === 0 ? (
                  ...
                  (() => {
                    ...
                    const stockData = item.symbol
                      ? newsStockPrices[item.symbol]
                      : null;
                    const isItemUp = stockData
                      ...
                      >
                        ...
                        {stockData && (
                          <div className="flex items-center gap-1.5">
                            ...
                            <span
                              className={`text-[9px] font-semibold shrink-0 ${isItemUp ? "text-emerald-400" : "text-red-400"}`}
                            >
                              {isItemUp ? "▲" : "▼"}{" "}
            ...
            <div className="absolute inset-0 flex items-stretch px-2.5 py-2 gap-2.5">
              ...
              <div className="flex-1 min-w-0 flex flex-col justify-center gap-1">
                ...
                <p className="text-[9px] leading-snug text-white/90 font-medium italic line-clamp-4">
                  "One health emergency can wipe out years of savings —
                  insurance is not a choice, it's a shield."
                </p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center gap-1.5 shrink-0">
                ...
                >
                  <span className="text-[9px] font-bold text-emerald-200 group-hover:text-white whitespace-nowrap transition-colors">
                    More Details
                  </span>
                  <span className="text-[7px] text-emerald-500/70 whitespace-nowrap">
                    irdai.gov.in
                  </span>
                </a>
      ...
      <Dialog open={isUrlDialogOpen} onOpenChange={setIsUrlDialogOpen}>
        <DialogContent>
          ...
          <div className="py-4">
            <Label htmlFor="stream-url" className="mb-2 block">
              YouTube URL
            </Label>
            <Input
              id="stream-url"
      ...
      {/* Tap zones — left edge = previous, right edge = next (narrow so inner content stays clickable) */}
      <div
